...
import 'package:example_resize_animated_sliver_box/resizable_items_model.dart';
...
class _ResisableItemsListState extends ConsumerState<ResizableItemsList> {
  ...
  Widget build(BuildContext context) {
    ResizableListState resizableListState = ref.watch(resizableItemProvider);

    return AnimatedSliverBox<ResizableItemsSliverBoxModel>(
      axis: resizableListState.axis,
      controllerSliverRowBox: resizableListState.sliverBoxController,
      createSliverRowBoxModel:
          (AnimatedSliverBoxState<ResizableItemsSliverBoxModel>
                  sliverBoxContext,
              Axis axis) {
        return ResizableItemsSliverBoxModel(
            duration: const Duration(milliseconds: 500),
            axis: axis,
            sliverBoxContext: sliverBoxContext,
            singleModel:
                SingleBoxModel<String, ResizableItemsSliverBoxProperties>(
                    tag: 'test',
                    items: resizableListState.colorList,
                    buildStateItem: _build));
      },
      updateSliverRowBoxModel: ((model, Axis axis) {
        model.axis = axis;

        for (SingleBoxModel<String, ResizableItemsSliverBoxProperties> single
  ...
  void actionEdit(
      ...
      required SingleBoxModel<String, ResizableItemsSliverBoxProperties> single,
      required ResizableItemsSliverBoxProperties properties}) {
    switch (action) {
      case ActionEdit.add:
        ref.read(resizableItemProvider.notifier).add(properties: properties);
        break;
...
class Normal extends StatelessWidget {
  ...
  final ResizableItemsSliverBoxProperties properties;
  final Axis axis;
