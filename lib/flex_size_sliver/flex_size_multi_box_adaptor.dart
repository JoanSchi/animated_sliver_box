...
abstract class FlexSizeRenderSliverMultiBoxAdaptor extends RenderSliver
    with
        ContainerRenderObjectMixin<RenderBox, FlexSizeSliverParentData>,
        RenderSliverHelpers,
        RenderSliverWithKeepAliveMixin {
  /// Creates a sliver with multiple box children.
  ///
  /// The [childManager] argument must not be null.
  FlexSizeRenderSliverMultiBoxAdaptor({
    required FlexHeightSliverRenderChildManager childManager,
  ...
  void setupParentData(RenderObject child) {
    if (child.parentData is! FlexSizeSliverParentData) {
      child.parentData = FlexSizeSliverParentData();
    }
  }
  ...
  /// [FlexSizeRenderSliverMultiBoxAdaptor] uses a [RenderSliverBoxChildManager] to
  /// create children during layout in order to fill the
  /// [SliverConstraints.remainingPaintExtent].
  @protected
  FlexHeightSliverRenderChildManager get childManager => _childManager;
  final FlexHeightSliverRenderChildManager _childManager;
  ...
  /// see [FlexSizeSliverParentData.keepAlive].
  ///
  ...
  bool addInitialChild({int index = 0, double layoutOffset = 0.0}) {
    ...
    if (firstChild != null) {
      ...
      final FlexSizeSliverParentData firstChildParentData =
  ...
  RenderBox? insertAndLayoutChild(
    ...
    final RenderBox? child = after == null ? firstChild : childAfter(after);
  ...
  @protected
  RenderBox? insertAndLayoutChildOriginal(
    BoxConstraints childConstraints, {
  ...
  ///
  /// Children whose [FlexSizeSliverParentData.keepAlive] property is
  ...
  void collectMyGarbage(
      {int leadingGarbage = -1,
      int trailingGarbage = -1,
      bool removeAll = false}) {
    assert(_debugAssertChildListLocked());

    assert(
        (!removeAll && leadingGarbage != -1 || trailingGarbage != -1) ||
            (removeAll && leadingGarbage == -1 || trailingGarbage == -1),
        removeAll
            ? 'LeadingGarbage and trailingGarbage should not be indexed, if removeall is true'
            : 'LeadingGarbage or trailingGarbage should be indexed, if removeall is false');

    invokeLayoutCallback<SliverConstraints>((SliverConstraints constraints) {
      int index = -1;
      while (firstChild != null &&
          (leadingGarbage > (index = indexOf(firstChild!)) || removeAll)) {
        // debugPrint('garbage index first ${indexOf(firstChild!)}');
        garbageCollectedIndex(index);
        _destroyOrCacheChild(firstChild!);
      }

      if (trailingGarbage != -1) {
        while (lastChild != null &&
            trailingGarbage < (index = indexOf(lastChild!))) {
          // debugPrint('garbage index last ${indexOf(lastChild!)}');
          garbageCollectedIndex(index);
          _destroyOrCacheChild(lastChild!);
        }
      }
  ...
  /// [FlexSizeSliverParentData.index] field of the child's [parentData].
...
class FlexSizeSliverParentData extends SliverLogicalParentData
...
abstract class FlexHeightSliverRenderChildManager {
